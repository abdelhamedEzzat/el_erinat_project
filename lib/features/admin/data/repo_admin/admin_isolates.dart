...
// This is for admin upload news
      case "uploadFamilyTree":
        bool success = await callbackForUploadFamilyTree(inputData);
        if (success) {
          print("upload family tree completed successfully");
          return Future.value(true);
        } else {
          print("upload family tree not completed");
          return Future.value(false);
        }
...
Future<bool> callbackForUploadFamilyTree(
  Map<String, dynamic>? inputData,
) async {
  try {
    await Firebase.initializeApp();
    String pdfName = inputData!['pdfName'];
    String familyName = inputData['familyName'];
    String familyLineage = inputData['familyLineage'];
    String pdfUrl = inputData['pdfUrl'];
    ...
    await TreeElerinatCubit(adminRepo: adminRepoImplementation)
