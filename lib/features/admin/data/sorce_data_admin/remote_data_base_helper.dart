...
class AdminRemoteDataBaseHelper {
  ...
  Future<void> uploadFilesToStorage({
    ...
    try {
      ...
      final imageName =
          'compressed_${DateTime.now().millisecondsSinceEpoch}_${imageFile.uri.pathSegments.last}';
      final pdfName =
          pdfFile.uri.pathSegments.last; // Use original file name for PDF
      ...
      final imageUploadTask =
          await imageStorageRef.putFile(compressedImageFile);
      final imageUploadSnapshot = await imageUploadTask;
      final imageUrl = await imageUploadSnapshot.ref.getDownloadURL();

      // Upload PDF to Firebase Storage
      final pdfStorageRef =
          FirebaseStorage.instance.ref().child('pdfsBook/$pdfName');
      final pdfUploadTask = pdfStorageRef.putFile(pdfFile);
      final pdfUploadSnapshot = await pdfUploadTask;
      final pdfUrl = await pdfUploadSnapshot.ref.getDownloadURL();

      // Listen for upload progress events
      pdfUploadTask.snapshotEvents.listen((TaskSnapshot snapshot) async {
      ...
      // Handle specific error scenarios or retry logic if needed
    }
  }
